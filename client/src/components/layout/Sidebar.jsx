...
const Sidebar = ({ className }) => {
  ...
  return (
    <div
      className={clsx(
        "flex flex-col h-full bg-white border-r border-board-border w-64",
        className,
      )}
    >
      <div className="p-6">
      ...
      <div className="mt-auto p-6 border-t border-board-border bg-board-bgSecondary/50">
        <div className="bg-white rounded-lg p-4 border border-board-border shadow-minimal">
          <p className="text-sm font-semibold text-board-heading mb-0.5">
            Plan:{" "}
            <span className="capitalize text-board-primary">
              {user?.plan || "Pro"}
            </span>
          </p>
          <p className="text-xs text-board-textSecondary mb-3">
            {user?.sessionsCount || 0} sessions used
          </p>
